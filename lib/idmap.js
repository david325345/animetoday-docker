...
function parseEpisodeAndSeason(fullId) {
  ...
  if (fullId.startsWith('kitsu:')) {
    if (parts.length >= 4) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  ...
  if (fullId.startsWith('anilist:')) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  // Prefixed IDs with the same <prefix>:<id>:<season>:<episode> shape.
  ...
  if (fullId.startsWith('mal:')) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  if (fullId.startsWith('anidb:')) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
