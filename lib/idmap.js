...
async function resolveIMDbViaChain(type, imdbId) {
  try {
    ...
    // Collect search names: original, stripped diacritics, English name
    const searchNames = new Set();
    if (meta.name) {
      searchNames.add(meta.name);
      searchNames.add(stripDiacritics(meta.name));
    }
    // Cinemeta sometimes has aliases/description with English name
    if (meta.nameEn) searchNames.add(meta.nameEn);
    if (meta.aliases) for (const a of meta.aliases) searchNames.add(a);

    // AniList lookup — try each name
