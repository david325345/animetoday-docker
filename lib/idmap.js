...
const xemCache = new Map(); // tvdbId → mapping data
...
async function resolveEpisode(fullId, season, episode) {
  ...
  // For IMDb/TVDB: try XEM mapping for season > 1
  if (season <= 1) return { absoluteEpisode: episode, season: 1 };

  const imdbId = fullId.split(':')[0];

  // Try XEM via TVDB ID (need to get TVDB from Cinemeta)
  try {
  ...
  // Fallback: try simple math via AniList episode counts
  // For now just return the season episode as-is
  console.log(`  📺 No XEM mapping, using S${season}E${episode} as-is`);
...
async function getTVDBId(imdbId) {
  try {
    ...
    // Cinemeta sometimes has tvdb_id
...
function findAbsoluteEpisode(mapping, season, episode) {
  // XEM mapping format: array of { season, episode, absolute } objects
  // or object with scene/tvdb/anidb keys
  if (!mapping) return null;

  // Try to find matching entry
  for (const entry of (Array.isArray(mapping) ? mapping : [])) {
    const tvdb = entry.tvdb || entry;
    const scene = entry.scene || entry;
    const anidb = entry.anidb;

    if (tvdb?.season == season && tvdb?.episode == episode) {
      // Return anidb absolute if available, otherwise scene absolute
      if (anidb?.absolute) return anidb.absolute;
      if (anidb?.episode) return anidb.episode;
      if (scene?.absolute) return scene.absolute;
    }
  }
