...
// ===== TheXEM episode mapping (S02E03 → absolute episode) =====
const xemCache = new Map();
const XEM_CACHE_TTL = 24 * 60 * 60 * 1000;

async function resolveEpisode(fullId, season, episode) {
  // For kitsu IDs, episode is already absolute
  if (fullId.startsWith('kitsu:')) return { absoluteEpisode: episode, season: 1 };

  // S01 → episode is already correct for single-season AniDB entries
  if (season <= 1) return { absoluteEpisode: episode, season: 1 };

  const imdbId = fullId.split(':')[0];

  // Method 1: Try XEM mapping
  try {
    const tvdbId = await getTVDBId(imdbId);
    if (tvdbId) {
      const mapping = await getXEMMapping(tvdbId);
      if (mapping && Array.isArray(mapping) && mapping.length > 0) {
        const abs = findAbsoluteEpisode(mapping, season, episode);
        if (abs && abs !== episode) {
          console.log(`  📺 XEM: S${String(season).padStart(2,'0')}E${String(episode).padStart(2,'0')} → absolute ep ${abs}`);
          return { absoluteEpisode: abs, season };
        }
      }
    }
  } catch {}

  // Method 2: Calculate offset using Cinemeta episode data
  try {
    ...
    // Count episodes in seasons before the requested season
    let offset = 0;
    for (let s = 1; s < season; s++) {
      const seasonEps = videos.filter(v => v.season === s);
      offset += seasonEps.length;
    }
    
    if (offset > 0) {
      const absoluteEp = offset + episode;
      console.log(`  📺 Cinemeta offset: S${season}E${episode} → absolute ep ${absoluteEp} (offset ${offset})`);
      return { absoluteEpisode: absoluteEp, season };
    }
  } catch {}

  // Method 3: Return season episode as-is (torrents might use SxxExx format)
  console.log(`  📺 No mapping found, using S${season}E${episode} as-is`);
  return { absoluteEpisode: episode, season };
}

async function getTVDBId(imdbId) {
  try {
    const resp = await axios.get(`https://v3-cinemeta.strem.io/meta/series/${imdbId}.json`, { timeout: 5000 });
    return resp.data?.meta?.tvdb_id || null;
  } catch { return null; }
}

async function getXEMMapping(tvdbId) {
  const cached = xemCache.get(tvdbId);
  if (cached && Date.now() - cached.timestamp < XEM_CACHE_TTL) return cached.data;

  try {
    const resp = await axios.get(`http://thexem.info/map/all`, {
      params: { id: tvdbId, origin: 'tvdb' }, timeout: 8000
    });
    const data = resp.data?.data;
    if (data && resp.data?.result === 'success') {
      xemCache.set(tvdbId, { data, timestamp: Date.now() });
      return data;
    }
  } catch {}

  xemCache.set(tvdbId, { data: null, timestamp: Date.now() });
  return null;
}

function findAbsoluteEpisode(mapping, season, episode) {
  if (!mapping || !Array.isArray(mapping)) return null;

  for (const entry of mapping) {
    const tvdb = entry.tvdb || {};
    const anidb = entry.anidb || {};
    const scene = entry.scene || {};

    if (parseInt(tvdb.season) === season && parseInt(tvdb.episode) === episode) {
      if (anidb.absolute) return parseInt(anidb.absolute);
      if (anidb.episode) return parseInt(anidb.episode);
      if (scene.absolute) return parseInt(scene.absolute);
    }
  }
...
module.exports = {
  ...
  resolveEpisode, weeklyUpdate, offlineDB, mappingCache
};
