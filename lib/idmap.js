...
function parseAnimeLists(xml) {
  ...
  console.log(`📋 Parsed ${count} anime-lists entries (${animeListsByTVDB.size} TVDB series)`);
}
...
// ===== Main resolver: Stremio ID → AniDB ID + episode =====
async function resolveToAniDB(type, fullId) {
  ...
  // For IMDb: resolve via Cinemeta → AniList → offline DB
