...
async function resolveIMDbViaChain(type, imdbId) {
  try {
    ...
    const anilistClient = require('./anilist-client');

    for (const searchName of searchNames) {
      if (!searchName) continue;
      try {
        const media = await anilistClient.searchByName(searchName);
        if (media?.length) {
...
function parseEpisodeAndSeason(fullId) {
  ...
  // Helper: parse season string allowing "0" (specials), but keep 1 as fallback for NaN
  const parseSeason = (s) => {
    const n = parseInt(s);
    return Number.isFinite(n) ? n : 1;
  };
  if (fullId.startsWith('at:')) {
    return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  }
  // Entry-based anime DBs (kitsu/mal/anilist/anidb): each season is a SEPARATE
  // entry, so the stream id carries NO season — catalogs send <prefix>:<id>:<episode>
  // (e.g. "kitsu:48242:1" = episode 1 of the S4 entry). Episode = LAST segment;
  // season is implied by the id itself and is NEVER sent to the indexer. If a
  // 4-part id ever arrives, its season segment is redundant and dropped.
  if (/^(?:kitsu|mal|anilist|anidb):/.test(fullId)) {
    return { season: null, episode: parts.length >= 3 ? (parseInt(parts[parts.length - 1]) || 1) : 1 };
  }
  // Season-based sources (tvdb/tmdb): <prefix>:<id>:<season>:<episode>.
  // Without these, "tmdb:297092:1:1" fell through to the un-prefixed branch
  // below and parsed season=297092 (the tmdb id!) → indexer 400.
  if (fullId.startsWith('tvdb:')) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  if (fullId.startsWith('tmdb:')) return { season: parseSeason(parts[2]), episode: parseInt(parts[3]) || 1 };
  if (parts.length >= 3) return { season: parseSeason(parts[1]), episode: parseInt(parts[2]) || 1 };
