...
//   (returns N streams, requires probing). SAB API is deterministic: one addfile with
//   cat=<jobName> lands in its own isolated folder /webdav/complete/<jobName>/, and a
//   PROPFIND of that folder returns exactly the file(s) for THIS job. Clean selection,
//   no aggregation, no probing.
//
// Flow per stream click:
//   1. fetchNzb(r2_url)              -> .nzb(.gz) from R2 + gunzip -> XML buffer
//   2. uploadNzb(buf, jobName)       -> POST /sabnzbd/api?mode=addfile&cat=<job> -> nzo_id
//   3. waitForCompleted(jobName)     -> poll mode=history until status === "Completed"
//   4. listVideoFiles(jobName)       -> PROPFIND /webdav/complete/<job>/ (Depth: infinity)
...
async function uploadNzb(nzbBuf, jobName) {
  ...
  const url = `${ALTMOUNT_URL}/sabnzbd/api?mode=addfile&apikey=${encodeURIComponent(API_KEY)}` +
    `&cat=${encodeURIComponent(jobName)}&nzbname=${encodeURIComponent(jobName)}&output=json`;
...
async function findInHistory(jobName) {
  ...
  // Prefer a Completed slot; among matches, prefer exact name, then category, then prefix.
...
async function listVideoFiles(jobName) {
  const folderUrl = `${ALTMOUNT_URL}${WEBDAV_BASE}/${encodeURIComponent(jobName)}/`;
  let resp;
  try {
