 ...
 *
 * Observed AltMount behaviour when we upload with nzbname=<jobName>:
 *   - history `name`    = our jobName (NOT renamed to release name, unlike ARR uploads)
 *   - history `storage` = /config/mount/complete/<Category>/<jobName>
 *   - WebDAV path       = /webdav/complete/<Category>/<jobName>/<jobName>.mkv
 *
 * So the WebDAV folder is: <WEBDAV_BASE>/<CATEGORY>/<jobName>/
 * (Category IS part of the WebDAV path here — different from the earlier assumption.)
 *
 * We derive the category from the slot when present (slot.category/cat), falling back
 * to the configured CATEGORY. The folder name is the slot `name` (= our jobName).
 */
function webdavFolderForSlot(slot) {
  const folderName = slot.name || slot.nzb_name?.replace(/\.nzb$/i, '') || '';
  if (!folderName) return null;
  const cat = slot.category || slot.cat || CATEGORY;
  const catPart = cat ? `${encodeURIComponent(cat)}/` : '';
  return `${WEBDAV_BASE}/${catPart}${encodeURIComponent(folderName)}/`;
}
