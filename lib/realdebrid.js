...
async function checkInstantAvailability(rdApiKey, hashes) {
  ...
  try {
    ...
    // Response: { "hash1": { "rd": [{ "file_id": { filename, filesize } }] }, "hash2": ... }
    // A hash is cached if it has "rd" array with at least one entry
    const result = {};
    for (const [hash, data] of Object.entries(resp.data || {})) {
      const rd = data?.rd;
      result[hash.toLowerCase()] = !!(rd && Array.isArray(rd) && rd.length > 0);
    }
    return result;
  } catch (err) {
