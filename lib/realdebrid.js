...
async function getRDStream(magnet, apiKey, episode = 0, isFileIdx = false) {
  if (!apiKey) return null;

  const cacheKey = getCacheKey(magnet, apiKey) + (isFileIdx ? `_fi${episode}` : episode ? `_ep${episode}` : '');
  const cached = rdStreamCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < RD_CACHE_TTL) return cached.url;

  let magnetHash = magnet.match(/btih:([a-zA-Z0-9]+)/i)?.[1] || '';
  // Convert to hex if base32 (RD stores hex hashes)
  if (magnetHash.length === 32 && /^[A-Za-z2-7]+$/.test(magnetHash)) {
    try { magnetHash = base32ToHex(magnetHash.toUpperCase()); } catch {}
  }
  magnetHash = magnetHash.toLowerCase();
  ...
  try {
    ...
    if (info.data?.links?.length) {
      const linkIdx = isFileIdx ? episode : findEpisodeLinkIndex(info.data, episode);
      const url = await unrestrictLink(apiKey, info.data.links[Math.min(linkIdx, info.data.links.length - 1)]);
      if (url) {
        console.log(`  RD: ✅ Ready (link ${linkIdx + 1}/${info.data.links.length}${isFileIdx ? ` fi${episode}` : episode ? ` ep${episode}` : ''})`);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (poll.data?.links?.length) {
        const linkIdx = isFileIdx ? episode : findEpisodeLinkIndex(poll.data, episode);
        const url = await unrestrictLink(apiKey, poll.data.links[Math.min(linkIdx, poll.data.links.length - 1)]);
        if (url) {
          console.log(`  RD: ✅ Ready (link ${linkIdx + 1}/${poll.data.links.length}${isFileIdx ? ` fi${episode}` : episode ? ` ep${episode}` : ''})`);
          rdStreamCache.set(cacheKey, { url, timestamp: Date.now() });
          return url;
        }
      }
...
// ===== Base32 to Hex conversion (for magnet hashes) =====
...
async function checkInstantAvailability(rdApiKey, hashes) {
  ...
  // Filter valid hex hashes — convert base32 to hex if needed
  const validHashes = hashes.map(h => {
    if (!h) return null;
    h = h.trim();
    if (/^[a-f0-9]{40}$/i.test(h)) return h.toLowerCase();
    if (/^[A-Z2-7]{32}$/i.test(h)) {
      try { return base32ToHex(h.toUpperCase()); } catch { return null; }
    }
    // Mixed case base32
    if (h.length === 32 && /^[a-zA-Z2-7]+$/.test(h)) {
      try { return base32ToHex(h.toUpperCase()); } catch { return null; }
    }
    return null;
  }).filter(Boolean);

  console.log(`  ⚡ RD check: ${hashes.length} input, ${validHashes.length} valid hex (sample: ${validHashes[0]?.substring(0,16)}... len=${validHashes[0]?.length})`);
  ...
  try {
    ...
    if (cachedCount > 0) console.log(`  ⚡ ${cachedCount} cached torrents found`);
    return result;
  } catch (err) {
    if (err.response?.status === 404) return {};
    if (err.response?.status === 403) {
      console.log(`  ⚡ RD 403 — token may be expired, skipping cache check`);
      return {};
    }
