...
async function getNamesFromIMDb(type, imdbId) {
  try {
    ...
    const validEnglish = best.title?.english && isLatinScript(best.title.english) && !isJunkTitle(best.title.english) ? best.title.english : null;
    const names = [name, validEnglish, validRomaji].filter(Boolean);
