...
// Written by lib/subs-index.js — on the persistent volume, not in public/,
// which the deploy wipes.
const SUBS_POSTERS_DIR = path.join(__dirname, '..', 'data', 'subs-posters');
...
// The back catalogue behind the separator. Same endpoint, different window:
// days=0 means "no time limit", and the service sorts by the newest subtitle
// per anime, so this is simply "everything we have, newest first". The archive
// (akihabara/TeamNS/NeoExorcist) is deliberately NOT part of it — the service
// leaves those out because they carry no first_seen.
const OLDER_LIMIT = parseInt(process.env.SUBS_OLDER_LIMIT) || 200;

async function fetchOlder() {
  try {
    const resp = await axios.get(`${SUBS_API_URL}/api/recent?days=0&limit=${OLDER_LIMIT}`, { timeout: 15000 });
    return resp.data?.items || [];
  } catch (err) {
    console.error(`  ❌ subs-added older fetch: ${err.message}`);
    return [];
  }
}
...
// Meta for an item behind the separator. Posters come from the subtitle search
// index (data/subs-posters/si_<imdb>.jpg, already carrying the CZ/SK tag) and
// fall back to MetaHub for anything the nightly index build has not covered
// yet — a plain poster is better than an empty tile.
function buildOlderMeta(item, baseUrl) {
  if (!item.imdb_id) return null;
  const local = path.join(SUBS_POSTERS_DIR, `si_${item.imdb_id}.jpg`);
  const poster = fs.existsSync(local)
    ? `${baseUrl}/subs-posters/si_${item.imdb_id}.jpg`
    : `https://images.metahub.space/poster/medium/${item.imdb_id}/img`;
  return {
    id: item.imdb_id,
    type: 'series',
    name: item.anime_title || 'Unknown',
    poster,
    description: item.langs ? `CZ/SK titulky (${item.langs})` : 'CZ/SK titulky',
  };
}

// Separator tile between "added today" and the back catalogue. Same id shape as
// the schedule's day separators, so the meta endpoint already answers null for
// it and a click does nothing.
function buildSeparatorMeta(baseUrl) {
  return {
    id: 'sep:older',
    type: 'series',
    name: 'Starší titulky',
    poster: `${baseUrl}/posters/sep_older.png`,
    description: 'Everything we have subtitles for, newest first.',
  };
}
