...
async function generateTodayAddedPosters(items) {
  ...
  for (const item of items) {
    ...
    const fakeSchedule = {
      ...
      topLabel: item.label,
      // Bottom: genre + rating from Cinemeta. Pass the FULL genres array so the
      // poster generator can pick a meaningful subgenre (Action/Fantasy/…) and
      // skip the generic "Animation"/"Anime" that every anime carries.
