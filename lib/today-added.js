...
const anilist = require('./anilist-client');
...
async function refreshTodayAdded() {
  ...
  // Enrich with AniList — uses shared rate limiter (800ms between requests)
  const enriched = [];
  for (const item of items) {
    const al = await anilist.fetchDetails(item.anilist_id);
