...
// ===== Filter calendar to today + N days =====
function filterSchedule(calendar, days = 3) {
  const now = new Date();
  // Start of today in JST (UTC+9) — SIMKL dates are in JST
  ...
  return calendar.filter(item => {
    ...
    return airDate >= todayDate && airDate < endDate;
  });
}

// ===== Get which day an item airs (0=today, 1=tomorrow, 2=day after) =====
function getAiringDay(dateStr) {
  const now = new Date();
  const todayStr = now.toLocaleDateString('en-CA', { timeZone: 'Asia/Tokyo' });
  const itemStr = new Date(dateStr).toLocaleDateString('en-CA', { timeZone: 'Asia/Tokyo' });
...
async function fetchAnimeSchedule() {
  ...
  for (const simklId of uniqueIds) {
    ...
    if (detail) detailCache.set(simklId, detail);
    await sleep(REQUEST_DELAY);

    // MAL rating
    const malRating = await fetchMALRating(simklId);
    ...
    if (anilistId) {
      ...
      if (alScore) {
        if (!ratingCache.has(`al:${simklId}`)) ratingCache.set(`al:${simklId}`, alScore);
      }
    }
    await sleep(REQUEST_DELAY);
  }
