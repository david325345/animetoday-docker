...
// ===== Filter calendar to today + N days (Tokyo/JST, matches SIMKL) =====
function filterSchedule(calendar, days = 3) {
  ...
  return calendar.filter(item => {
    ...
    const airPragueStr = airDate.toLocaleDateString('en-CA', { timeZone: 'Asia/Tokyo' });
    const airPragueDate = new Date(airPragueStr + 'T00:00:00');
    return airPragueDate >= todayDate && airPragueDate < endDate;
  });
}

// ===== Get which day an item airs (0=today, 1=tomorrow, 2=day after) in Tokyo/JST =====
function getAiringDay(dateStr) {
  const now = new Date();
  const todayStr = now.toLocaleDateString('en-CA', { timeZone: 'Europe/Prague' });
  const itemPragueStr = new Date(dateStr).toLocaleDateString('en-CA', { timeZone: 'Asia/Tokyo' });
...
async function fetchAnimeSchedule() {
  ...
  const schedules = filtered.map(item => {
    ...
    const tvdbSeason = epEntry?.tvdb?.season || siblingSeason || detail?.mapped_tvdb_seasons?.[0] || 1;
