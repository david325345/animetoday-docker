...
// Retries on transient errors (DNS EAI_AGAIN, ETIMEDOUT, ECONNRESET, ENOTFOUND, 5xx).
async function fetchCalendar() {
  const RETRY_ERRORS = ['EAI_AGAIN', 'ETIMEDOUT', 'ECONNRESET', 'ENOTFOUND', 'ECONNREFUSED'];
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.log(`📅 SIMKL calendar: ${items.length} entries${attempt > 1 ? ` (attempt ${attempt})` : ''}`);
      return items;
    } catch (err) {
      const code = err.code || '';
      const status = err.response?.status || 0;
      const isRetryable = RETRY_ERRORS.includes(code) || status >= 500;

      if (isRetryable && attempt < MAX_ATTEMPTS) {
        const wait = attempt * 2000; // 2s, 4s, 6s
        console.warn(`⚠️ SIMKL calendar attempt ${attempt}/${MAX_ATTEMPTS}: ${err.message} — retrying in ${wait}ms`);
        await sleep(wait);
        continue;
      }
...
// ===== Filter calendar to today + N days (Europe/Prague, matches SIMKL web) =====
...
// ===== Get which day an item airs (0=today, 1=tomorrow, 2=day after) in Europe/Prague =====
...
function getDayLabel(dayOffset) {
  ...
  const dayName = date.toLocaleDateString('en-GB', { timeZone: 'Europe/Prague', weekday: 'long' });
  const dateStr = date.toLocaleDateString('en-GB', { timeZone: 'Europe/Prague', day: 'numeric', month: 'numeric' });

  if (dayOffset === 1) return `Tomorrow · ${dayName} ${dateStr}`;
  if (dayOffset === 2) return `Day after · ${dayName} ${dateStr}`;
...
// ===== Fetch anime episodes list (per-episode TVDB season/number mapping) =====
// Endpoint: /anime/episodes/{simklId} — returns array of episodes
// Each episode has: { episode, title, date, aired, ids: { simkl_id }, tvdb: { season, episode } }
// `tvdb.season` and `tvdb.episode` give us the GLOBAL TVDB numbering across all cours.
// E.g. Dr. Stone Science Future Cour 3 Ep 6 → tvdb: { season: 4, episode: 30 }
async function fetchAnimeEpisodes(simklId) {
  try {
    const resp = await axios.get(`${API_BASE}/anime/episodes/${simklId}`, {
      params: { client_id: SIMKL_CLIENT_ID },
      timeout: 10000
    });
    return Array.isArray(resp.data) ? resp.data : [];
  } catch (err) {
    console.error(`  ❌ SIMKL episodes ${simklId}: ${err.message}`);
    return [];
  }
}
...
async function fetchAnimeSchedule() {
  ...
  // 4. Fetch details + ratings + per-episode TVDB mapping for each unique anime
  const detailCache = new Map();
  const ratingCache = new Map();
  const episodesCache = new Map(); // simklId → episodes array (with tvdb.season/episode)

  for (const simklId of uniqueIds) {
    // Fetch detail + episodes in parallel — they are independent endpoints
    const [detail, episodes] = await Promise.all([
      fetchAnimeDetail(simklId),
      fetchAnimeEpisodes(simklId),
    ]);

    if (episodes.length) {
      episodesCache.set(simklId, episodes);
    }
    ...
    // AniList score — taken from SIMKL detail.ratings (no direct AniList call)
    if (detail?.ratings?.anilist?.rating) {
      ratingCache.set(`al:${simklId}`, parseFloat(detail.ratings.anilist.rating).toFixed(1));
    }
    await sleep(REQUEST_DELAY);
  }

  console.log(`📅 Details: ${detailCache.size}/${uniqueIds.length}, episodes: ${episodesCache.size}/${uniqueIds.length}, MAL ratings: ${ratingCache.size}`);
  ...
  const schedules = filtered.map(item => {
    ...
    // ===== Resolve TVDB-style season + episode =====
    // SIMKL splits anime into per-cour entries, so calendar `episode` is per-cour (1-12).
    // We want the GLOBAL TVDB numbering (e.g. Cour 3 Ep 6 → S04E30).
    // Strategy: look up episode in /anime/episodes/{simklId} response by SIMKL's per-cour number.
    // Fallback: mapped_tvdb_seasons[0] for season, raw SIMKL episode for episode.
    const simklEpisode = item.episode?.episode || 1;
    const episodes = episodesCache.get(simklId);
    const epEntry = episodes?.find(e => e.episode === simklEpisode);

    // Season: prefer this episode's TVDB season. A brand-new episode often has no
    // TVDB mapping yet → borrow the season from any sibling episode in the same
    // per-cour list (they all share one TVDB season), then fall back to detail/1.
    const siblingSeason = episodes?.find(e => e.tvdb?.season)?.tvdb?.season;
    let tvdbSeason = epEntry?.tvdb?.season || siblingSeason || detail?.mapped_tvdb_seasons?.[0] || 1;
    // SIMKL sometimes maps a brand-new sequel cour to TVDB season 1 (stale mapping),
    // even though it knows the title (e.g. "... season 2"). If we resolved season 1
    // but the title explicitly says "season N" (N>1), trust the title.
    if (tvdbSeason === 1) {
      const titleText = `${detail?.en_title || ''} ${item.title || ''}`;
      const m = titleText.match(/\bseason\s+(\d+)\b|\b(\d+)(?:st|nd|rd|th)\s+season\b/i);
      const titleSeason = m ? parseInt(m[1] || m[2], 10) : 0;
      if (titleSeason > 1) tvdbSeason = titleSeason;
    }
    const tvdbEpisode = epEntry?.tvdb?.episode || simklEpisode;

    if (epEntry?.tvdb?.episode && epEntry.tvdb.episode !== simklEpisode) {
      console.log(`  🎬 ${item.title} Ep${simklEpisode} → TVDB S${String(tvdbSeason).padStart(2, '0')}E${String(tvdbEpisode).padStart(2, '0')}`);
    }
    ...
    return {
      ...
      // TVDB-numbered episode (global across all cours). Used for IMDb stream lookups.
      episode: tvdbEpisode,
      // TVDB season (e.g. Dr.Stone Cour 3 Ep 6 → season 4, episode 30)
      season: tvdbSeason,
      // Original SIMKL per-cour episode number (kept for debugging / SIMKL deeplinks)
      simklEpisode,
      // Year of first airing (Fusion catalog meta + display)
