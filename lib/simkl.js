...
async function fetchAnimeSchedule() {
  ...
  const schedules = filtered.map(item => {
    ...
    return {
      ...
      // TVDB-style season from SIMKL detail (mapped_tvdb_seasons array)
      // Fallback to 1 if not provided. Used in poster pill (S03E06 etc.)
      season: detail?.mapped_tvdb_seasons?.[0] || 1,
