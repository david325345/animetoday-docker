...
const POSTER_W = 500, POSTER_H = 750, RADIUS = 18;
...
async function buildPoster(imdb, label) {
  const out = path.join(POSTERS_DIR, `si_${imdb}.png`);
  if (fs.existsSync(out)) return `/posters/si_${imdb}.png`;   // already done
  // large (780x1170) downscales better than medium and ends up smaller
  const url = `${METAHUB_URL}/poster/large/${imdb}/img`;
  const resp = await axios.get(url, { responseType: 'arraybuffer', timeout: 15000 });

  const rounded = Buffer.from(`<svg width="${POSTER_W}" height="${POSTER_H}" xmlns="http://www.w3.org/2000/svg"><rect width="${POSTER_W}" height="${POSTER_H}" rx="${RADIUS}" ry="${RADIUS}" fill="white"/></svg>`);
  const base = await sharp(Buffer.from(resp.data))
