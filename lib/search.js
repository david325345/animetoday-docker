...
async function fetchAllForAid(anidbId) {
  ...
  try {
    // Fetch without order limit to get ALL torrents, not just top seeders
    const resp = await axios.get('https://feed.animetosho.org/json', {
      params: { aid: anidbId, limit: 0 }, timeout: 20000
    });
...
async function searchByAniDBId(anidbId, episode, isMovie = false) {
  ...
  if (episode && !isMovie) {
    ...
    if (targetEid) {
      // Split items into: with eid, without eid
      const withEid = items.filter(i => i.anidb_eid && i.anidb_eid > 0);
      const withoutEid = items.filter(i => !i.anidb_eid || i.anidb_eid === 0);

      // Exact eid match from items that have eid
      const eidMatches = withEid.filter(i => i.anidb_eid === targetEid);

      // For items WITHOUT eid: use regex matching + batch detection
      const regexMatches = withoutEid.filter(item => {
      ...
      filtered = [...eidMatches, ...regexMatches];
      console.log(`  [AnimeTosho] eid=${targetEid}: ${eidMatches.length} exact + ${regexMatches.length} regex/batch = ${filtered.length} (${withoutEid.length} items had no eid)`);
    } else {
