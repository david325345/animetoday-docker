...
async function searchByAniDBId(anidbId, episode, isMovie = false) {
  ...
  if (episode && !isMovie) {
    ...
    console.log(`  [AnimeTosho] After ep${ep} filter: ${filtered.length} (with magnet: ${filtered.filter(i=>i.magnet_uri).length}, with hash: ${filtered.filter(i=>i.info_hash).length})`);
  }

  // Deduplicate by hash
  const seen = new Set();
  filtered = filtered.filter(item => {
    // Get hash from magnet_uri or info_hash field
...
function matchesEpisodeNum(title, ep) {
  // SxxExx
  if (new RegExp(`S\\d{1,2}E0*${ep}\\b`, 'i').test(title)) return true;
  // " - 08 " fansub style
  ...
  // 3-digit: " 036 "
  if (ep >= 100 || String(ep).padStart(3, '0').length === 3) {
