...
async function searchByAniDBId(anidbId, episode, isMovie = false, noCache = false) {
  ...
  if (episode && !isMovie) {
    ...
    if (eid) {
      ...
      // Also fetch batch torrents from aid search
      let batchResults = [];
      try {
        ...
        batchResults = aidItems
          .filter(i => {
            if (!(i.magnet_uri || i.info_hash)) return false;
            // Batch = torrent with multiple files (3+)
            if (i.num_files && i.num_files >= 3) return true;
            return false;
          })
          .map(i => {
            if (!i.magnet_uri && i.info_hash) i.magnet_uri = `magnet:?xt=urn:btih:${i.info_hash}`;
      ...
      if (batchResults.length) console.log(`  📦 +${batchResults.length} batch (${deduped.length} total)`);
...
const DEFAULT_GROUPS = ['SubsPlease', 'Erai-raws', 'EMBER', 'ASW', 'Judas', 'AnimeRG', 'Anime Time', 'Mysteria', 'Yameii', 'ToonsHub', 'VARYG', 'Golumpa', 'New-raws', 'DeadFish', 'Hi10', 'Coalgirls'];
...
function sortByGroupPriority(torrents, userPrefs = null) {
  ...
  // Sort mode: 'qualityThenSeeders' | 'qualityThenSize' | 'seeders' | 'size' | 'custom'
  // Falls back to legacy customSortEnabled boolean for backward compatibility.
  let mode = userPrefs?.sortMode;
  if (!mode) {
    mode = userPrefs?.customSortEnabled ? 'custom' : 'qualityThenSeeders';
  }

  const resOrder = userPrefs?.resPriority || DEFAULT_RESOLUTIONS;
  const getResRank = (t) => {
    const res = t.seadex ? 'SeaDex' : detectResolution(t.name);
  ...
  // Size: prefer per-episode matchedFile.size (correct for batches), then raw filesizeBytes,
  // fallback on parsing the formatted filesize string (legacy code paths).
  const getSize = (t) => {
  ...
  if (mode === 'qualityThenSeeders') {
    return [...withMagnet].sort((a, b) => {
      const r = getResRank(a) - getResRank(b);
      if (r !== 0) return r;
  ...
  if (mode === 'seeders') {
    return [...withMagnet].sort((a, b) => getSeeders(b) - getSeeders(a));
  }
  if (mode === 'size') {
    return [...withMagnet].sort((a, b) => getSize(b) - getSize(a));
  }

  // mode === 'custom' → use existing advanced sort below
  ...
  const sortBySeeders = userPrefs.sortBySeeders !== false;

  // Filter excluded resolutions (don't exclude SeaDex)
  let filtered = withMagnet.filter(t => {
    if (t.seadex) return true; // never exclude SeaDex
    const res = detectResolution(t.name);
  ...
  // Sort
  filtered.sort((a, b) => {
    // Resolution priority — SeaDex uses "SeaDex" as virtual resolution tag
    const aRes = a.seadex ? 'SeaDex' : detectResolution(a.name);
    const bRes = b.seadex ? 'SeaDex' : detectResolution(b.name);
    const aGroup = detectGroup(a.name);
    const bGroup = detectGroup(b.name);
    ...
    // Within SeaDex: isBest first
    if (a.seadex && b.seadex) {
      if (a.isBest !== b.isBest) return a.isBest ? -1 : 1;
    }

    // Group priority
    const aGrpIdx = aGroup ? groupOrder.indexOf(aGroup) : -1;
    const bGrpIdx = bGroup ? groupOrder.indexOf(bGroup) : -1;
    const aGrpPri = aGrpIdx >= 0 ? aGrpIdx : groupOrder.length;
    const bGrpPri = bGrpIdx >= 0 ? bGrpIdx : groupOrder.length;
    if (aGrpPri !== bGrpPri) return aGrpPri - bGrpPri;

    // Seeders
    if (sortBySeeders) return (parseInt(b.seeders) || 0) - (parseInt(a.seeders) || 0);
    return 0;
  });
...
function defaultSort(torrents) {
  const top = torrents.filter(t => ['SubsPlease', 'Erai-raws'].some(g => (t.name || '').includes(g)) && /1080p/i.test(t.name || ''))
    .sort((a, b) => (parseInt(b.seeders) || 0) - (parseInt(a.seeders) || 0));
  const topSet = new Set(top.map(t => t.magnet));
  const rest = torrents.filter(t => !topSet.has(t.magnet))
    .sort((a, b) => {
...
module.exports = { searchByAniDBId, searchByText, detectQuality: detectResolution, sortByGroupPriority, loadEidCache, DEFAULT_GROUPS, DEFAULT_RESOLUTIONS };
