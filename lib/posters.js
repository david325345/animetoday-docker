...
function pickGenre(genres) {
  ...
  // Only generic "Animation"/"Anime" available → show NO genre (just rating).
  // Every title here is anime, so "Animation" is redundant noise.
...
// ===== Empty-state card (catalog row with no items today) =====
// Same visual language as the sep_day cards: bgGrad + white top pill, but with a
// centered message instead of date/swipe hint. Files are ph_*.png — preserved by
// the cleanup above and regenerated on startup.
async function generateEmptyCard(filename, pillText, lines) {
  const outputPath = path.join(POSTERS_DIR, `ph_${filename}.png`);
  try {
    const W = POSTER_W, H = POSTER_H;
    const charW = PILL_FONT_SIZE * 0.55;
    const textW = pillText.length * charW;
    const pillW = Math.round(textW + 80);
    const pillX = Math.round((W - pillW) / 2);
    const r = PILL_H / 2;
    const pillPath = `M ${pillX} 0 L ${pillX + pillW} 0 L ${pillX + pillW} ${PILL_H - r} Q ${pillX + pillW} ${PILL_H} ${pillX + pillW - r} ${PILL_H} L ${pillX + r} ${PILL_H} Q ${pillX} ${PILL_H} ${pillX} ${PILL_H - r} Z`;
    ...
    const lineSVG = lines.map((l, i) =>
      `<text x="${W / 2}" y="${cy + (i - (lines.length - 1) / 2) * 58}" font-family="Poppins, DejaVu Sans, sans-serif" font-size="44" font-weight="600" fill="#8f8fa8" text-anchor="middle">${escapeXml(l)}</text>`
    ).join('');
    const svg = `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="${W}" height="${H}" fill="url(#bgGrad)" rx="${RADIUS}" ry="${RADIUS}"/>
      <path d="${pillPath}" fill="#ffffff"/>
      <text x="${W / 2}" y="${PILL_H / 2 + PILL_FONT_SIZE / 3}" font-family="Poppins, DejaVu Sans, sans-serif" font-size="${PILL_FONT_SIZE}" font-weight="700" fill="#1a1a25" text-anchor="middle">${escapeXml(pillText)}</text>
      ${lineSVG}
    </svg>`;
    await sharp(Buffer.from(svg)).png({ quality: 92 }).toFile(outputPath);
    return `/posters/ph_${filename}.png`;
  } catch (err) {
    console.error(`  🖼️ Empty card ${filename}: ${err.message}`);
    return null;
  }
}

// ===== Separator card for the "older subtitles" part of subs-added =====
// Same visual language as the sep_day cards (bgGrad + white top pill), used to
// split "added today" from the back catalogue inside one catalogue row.
async function generateOlderSeparator() {
  const outputPath = path.join(POSTERS_DIR, 'sep_older.png');
  try {
    if (fs.existsSync(outputPath)) return '/posters/sep_older.png';   // static, draw once

    // Own SVG rather than buildPlaceholderSVG: that one always draws the white
    // top pill, which does not fit a card whose whole point is a plain heading.
    const cx = POSTER_W / 2;
    const accent = '#E84363';
    const font = 'Poppins, DejaVu Sans, sans-serif';
    const y = POSTER_H - 90;
    const line = (t, ty, size, fill, weight = 700) =>
      `<text x="${cx}" y="${ty}" font-family="${font}" font-size="${size}" font-weight="${weight}" fill="${fill}" text-anchor="middle">${escapeXml(t)}</text>`;

    const svg = `<svg width="${POSTER_W}" height="${POSTER_H}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="bgGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#282346"/>
          <stop offset="100%" stop-color="#0F0F1E"/>
      ...
      <rect width="${POSTER_W}" height="${POSTER_H}" fill="url(#bgGrad)" rx="${RADIUS}" ry="${RADIUS}"/>
      ${line('Starší', 120, 54, '#ffffff')}
      ${line('titulky', 180, 54, '#ffffff')}
      ${line('CZ/SK', POSTER_H / 2 + 40, 120, accent)}
      <line x1="${cx - 80}" y1="${y}" x2="${cx + 60}" y2="${y}" stroke="${accent}" stroke-width="8" stroke-linecap="round"/>
      <polygon points="${cx + 50},${y - 22} ${cx + 100},${y} ${cx + 50},${y + 22}" fill="${accent}"/>
    </svg>`;
    await sharp(Buffer.from(svg)).png({ quality: 92 }).toFile(outputPath);
    return '/posters/sep_older.png';
  } catch (err) {
    console.error(`  🖼️ Older separator: ${err.message}`);
    return null;
  }
}
...
async function generateAllPosters(schedules) {
  ...
  // Clean only anime-today files (sep_day*.png + numeric simklId.png) — keep
  // ta_* (today-added) AND sa_* (subs-added): both are managed by their own
  // modules with their own stale-cleanup; wiping them here caused 404 posters
  // in the subs-added catalog after every schedule refresh (2026-07-12).
  try {
    for (const f of fs.readdirSync(POSTERS_DIR)) {
      if (f.startsWith('ta_') || f.startsWith('sa_') || f.startsWith('ph_') || f.startsWith('si_') || f === 'sep_older.png') continue; // preserve today-added + subs-added + placeholder + subs-index + older separator
...
module.exports = {
  generateOlderSeparator, generateAllPosters, generatePoster, generatePlaceholder, generateEmptyCard, formatTimeCET };
