...
// Symmetric: panel extends as far above text as text is from poster bottom edge
const FROSTED_PANEL_PCT = 0.22;
const FROSTED_BLUR = 12;
const FROSTED_DARKEN_ALPHA = 0.40;
const FROSTED_FADE_PCT = 0.40;
...
async function buildFrostedPanel(posterBuf) {
  ...
  const fadeStopPct = (FROSTED_FADE_PCT * 100).toFixed(0);
  const maskSvg = Buffer.from(
    `<svg width="${POSTER_W}" height="${panelH}" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="rgb(0,0,0)"/><stop offset="${fadeStopPct}%" stop-color="rgb(255,255,255)"/><stop offset="100%" stop-color="rgb(255,255,255)"/></linearGradient></defs><rect width="${POSTER_W}" height="${panelH}" fill="url(#g)"/></svg>`
  );

  const masked = await sharp(darkened)
    .ensureAlpha()
