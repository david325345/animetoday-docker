...
async function generateAllPosters(schedules) {
  ...
  try {
    for (const f of fs.readdirSync(POSTERS_DIR)) {
      if (f.startsWith('ta_') || f.startsWith('sa_') || f.startsWith('ph_') || f.startsWith('si_')) continue; // preserve today-added + subs-added + placeholder + subs-index posters
...
module.exports = { generateAllPosters, generatePoster, generatePlaceholder, generateEmptyCard, formatTimeCET };
