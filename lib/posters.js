...
function pickGenre(genres) {
  ...
  // Skip generic "Animation" (every anime has it) — pick next meaningful genre
  const filtered = genres.filter(g => g !== 'Animation');
...
const FROSTED_PANEL_PCT = 0.34;    // panel height (top edge fades smoothly)
