...
function filterJunk(torrents, animeName) {
  const junkPattern = /\bmini anime\b|\bmini-anime\b|\brecap\b|\bncop\b|\bnced\b|\bpv\b|\bpreview\b|\btrailer\b/i;
  // OVA/Special only rejected if standalone (not part of batch with series)
  const ovaStandalonePattern = /\bova\b|\bspecial\b/i;
  const batchIndicator = /\bcomplete\b|\bbatch\b|\bcollection\b|\d+\s*[-~]\s*\d+|\bS\d+\s*\+/i;

  let filtered = torrents.filter(t => {
    const name = t.name || '';
    if (junkPattern.test(name)) {
      console.log(`  [Junk] REMOVED (pattern): ${name.substring(0, 80)}`);
      return false;
    }
    // OVA/Special: only remove if it's standalone, not part of a batch
    if (ovaStandalonePattern.test(name) && !batchIndicator.test(name)) {
      console.log(`  [Junk] REMOVED (standalone OVA/Special): ${name.substring(0, 80)}`);
      return false;
    }
    return true;
  });
  ...
  filtered = filtered.filter(t => {
    ...
    // Allow file extensions, quality/tech terms, common torrent words
    if (/^(mkv|mp4|avi|flv|wmv|webm|srt|ass|flac|aac|opus|ac3|dts)$/i.test(nextWord)) return true;
    if (/^(bd|bluray|dvd|web|webrip|webdl|hdtv|bdrip|remux|hevc|avc|x264|x265|av1|hi10p|10bit|8bit)$/i.test(nextWord)) return true;
    if (/^(1080p|720p|480p|2160p|4k|uhd|hd|sd|fullhd)$/i.test(nextWord)) return true;
    if (/^(dual|multi|eng|jpn|raw|sub|dub|dubbed|subbed|hardsub|softsub|vostfr|multi)$/i.test(nextWord)) return true;
    if (/^(kimi|boku|watashi|ore|sono|ano|kono|yoru|hoshi|kami|mahou|sensei|kun|chan|san|sama|senpai)$/i.test(nextWord)) return true;
    if (/^v\d/.test(nextWord)) return true; // v2, v3 etc
    if (/^\(/.test(nextWord) || /^\[/.test(nextWord)) return true; // starts with bracket
    if (/^\d$/.test(nextWord)) { console.log(`  [Junk] REMOVED (single digit "${nextWord}"): ${rawName.substring(0, 80)}`); return false; }
    // Only reject for very long names (5+ words) and only if it looks like a different title
    if (/^[a-z]{4,}$/i.test(nextWord) && searchWords.length >= 5) { console.log(`  [Junk] REMOVED (word after "${nextWord}"): ${rawName.substring(0, 80)}`); return false; }
    return true;
  });
...
async function searchNyaa(names, episode, season = 1) {
  ...
  // Collect Nyaa results FIRST (priority for source labeling)
  for (const t of nyaaResults) {
    const hash = t.magnet?.match(/btih:([a-zA-Z0-9]+)/i)?.[1]?.toLowerCase();
    if (hash && !seenHashes.has(hash)) {
      seenHashes.add(hash);
      if (!t.source) t.source = 'nyaa';
      allTorrents.push(t);
    }
  }

  // Collect AnimeTosho results (only new hashes)
  for (const t of toshoResults) {
    const hash = t.magnet?.match(/btih:([a-zA-Z0-9]+)/i)?.[1]?.toLowerCase();
    if (hash && !seenHashes.has(hash)) {
      seenHashes.add(hash);
      allTorrents.push(t); // already has source: 'animetosho'
    }
  }
    ...
    uniqueQueries.map(q =>
      axios.get('https://nyaa.si/', {
        ...
        while ((match = rowRegex.exec(html)) !== null) {
          const row = match[1];
          // Extract magnet from this specific row
          const magnetMatch = row.match(/href="(magnet:\?xt=urn:btih:[^"]+)"/);
          if (!magnetMatch) continue;
          // Extract name: last <a> with href="/view/..."
          const nameMatches = [...row.matchAll(/<a[^>]*href="\/view\/\d+"[^>]*>([^<]+)<\/a>/g)];
          const name = nameMatches.length ? nameMatches[nameMatches.length - 1][1].trim() : '';
          // Extract all text-center td values: [size, date, seeders, leechers, downloads]
          const tdValues = [...row.matchAll(/<td[^>]*class="text-center"[^>]*>([^<]*)<\/td>/g)].map(m => m[1].trim());
          // Nyaa column order: size(0), date(1), seeders(2), leechers(3), downloads(4)
          // But some rows may vary — find size by pattern, seeders/leechers by position
          const sizeVal = tdValues.find(v => /\d+[\d.]*\s*[KMGT]iB/.test(v)) || 'Unknown';
          // Seeders and leechers are the last numeric-only values
          const numericTds = tdValues.filter(v => /^\d+$/.test(v));
          // Typically: seeders, leechers, downloads (3 numbers)
          const seeders = numericTds.length >= 3 ? numericTds[0] : (numericTds[0] || '0');
