...
function filterJunk(torrents, animeName) {
  ...
  // If search name is very short (1-2 words), be strict about false matches
  const isShortName = searchWords.length <= 2;

  filtered = filtered.filter(t => {
    const rawName = (t.name || '');
    // Strip fansub tags in brackets first, then normalize
    const stripped = rawName.replace(/\[[^\]]*\]/g, ' ').replace(/\([^)]*\)/g, ' ');
    const torrentNorm = stripped.toLowerCase().replace(/[;:]/g, ' ').replace(/[\[\]()_.\-]/g, ' ').replace(/\s+/g, ' ').trim();

    // For short names: check the title part is a standalone match, not embedded in another title
    if (isShortName) {
      // Build regex: search name must appear as whole words (word boundary)
      const escaped = searchNameNorm.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const re = new RegExp(`(?:^|\\s)${escaped}(?:\\s|$|\\d)`, 'i');
      if (!re.test(torrentNorm)) {
        // Name not found as standalone → check if it's part of a bigger title
        // e.g. "steins gate" contains "gate" but not as standalone
        return false;
      }

      // Also check nothing alphabetic is directly adjacent (part of compound title)
      const idx = torrentNorm.indexOf(searchNameNorm);
      if (idx > 0) {
        const charBefore = torrentNorm[idx - 1];
        if (charBefore && charBefore !== ' ') return false;
        // Check the word before is not a title word (alphabetic, not a common tag)
        ...
        if (wordBefore.length >= 2 && /^[a-z]+$/.test(wordBefore) &&
            ...
            !['new', 'the', 'a', 'an', 'of', 'in', 'at', 'on', 'to', 'for', 'vs', 'and', 'or'].includes(wordBefore)) {
          // Word before looks like part of a different title (e.g. "steins" before "gate")
          // But only reject if the word is NOT part of any of the search names
          const allSearchNames = animeName.toLowerCase();
          if (!allSearchNames.includes(wordBefore)) {
            return false;
          }
        }
      }
    ...
    // Standard check: what's after the name
    const idx = torrentNorm.indexOf(searchNameNorm);
    ...
    if (/^[a-z]{2,}$/i.test(nextWord) && !isShortName) return false;
    return true;
  });
