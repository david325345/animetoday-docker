...
function filterMovieResults(torrents, names) {
  // Build normalized versions of all movie names
  const movieNames = names.map(n => n.toLowerCase().replace(/[;:.,!?'"]/g, '').replace(/\s+/g, ' ').trim());
  // Find the longest name (most specific, e.g. "goblin slayer goblins crown")
  const longestName = movieNames.reduce((a, b) => a.length >= b.length ? a : b, '');
  const longestWords = longestName.split(/\s+/);
  ...
  return torrents.filter(t => {
    ...
    // Reject torrents with episode patterns (these are series, not movies)
    // e.g. "- 01", "E01", "EP01", "S01E01", "Episode 1"
    if (/\bS\d+E\d+\b/i.test(name)) return false;
    if (/\bEP?\s*\d{1,3}\b/i.test(name) && !/\b(EP|episode)\b/i.test(longestName)) return false;
    if (/\bEpisode\s+\d+/i.test(name)) return false;
    // Pattern: " - 01 " or " - 1 " (common series episode format)
    if (/\s-\s\d{1,3}\s/.test(name) || /\s-\s\d{1,3}[.\[v]/.test(name)) return false;
    // Standalone small number at end after space (like "Goblin Slayer 03")
    if (/\s\d{1,2}(?:\s*[\[\(]|\s*$)/.test(name.replace(/\[[^\]]*\]/g, '').trim())) {
      // But allow if the number looks like a year (2019, 2024)
      const numMatch = name.match(/\s(\d{1,4})(?:\s*[\[\(]|\s*$)/);
      if (numMatch && parseInt(numMatch[1]) < 1900) return false;
    }

    // Prefer torrents that contain more words from the full movie title
    // e.g. "Goblin Slayer Goblin's Crown" should score higher than just "Goblin Slayer"
    if (longestWords.length >= 3) {
      const matchCount = longestWords.filter(w => w.length > 2 && nameNorm.includes(w)).length;
      const matchRatio = matchCount / longestWords.length;
      // If less than 50% of movie title words match, likely wrong result
