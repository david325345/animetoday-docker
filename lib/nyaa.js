...
function filterJunk(torrents, animeName) {
  ...
  filtered = filtered.filter(t => {
    ...
    // Check what's BEFORE the name — if there's a non-separator word, it's a different title
    // e.g. "steins gate" when searching "gate" → "steins" is before → reject
    const beforeName = torrentNorm.slice(0, idx).trim();
    if (beforeName) {
      const lastWordBefore = beforeName.split(/\s+/).pop() || '';
      // If the word before is a fansub group tag, number, or bracket content, allow it
      // But if it's a regular word (part of another title), reject
      if (lastWordBefore && /^[a-z]{2,}$/i.test(lastWordBefore) &&
          !batchWords.has(lastWordBefore) && !particleWords.has(lastWordBefore)) {
        return false;
      }
    }
