// lib/nzbdav.js
// Client for NzbDav (mount-only NZB streaming server with SAB-compatible API + WebDAV).
//
// Flow per stream click:
//   1. fetchNzb(r2_url)            → download .nzb.gz from R2 + gunzip → plain XML buffer
//   2. uploadNzb(buf, jobName)     → POST to SAB API, NzbDav stores under jobName
//   3. waitForCompleted(jobName)   → poll history until status === Completed
//   4. listVideoFiles(jobName)     → PROPFIND /content/stremio/<jobName>/ for .mkv/.mp4
//   5. proxyStream(req, res, jobName, file) → range-proxy WebDAV bytes back to client
//
// Idempotency: jobName is a deterministic hash of r2_url. Re-clicking same stream
// finds existing completed job in history immediately (skip steps 1-3).

const axios = require('axios');
const crypto = require('crypto');
const zlib = require('zlib');
const FormData = require('form-data');
const { URL } = require('url');

const NZBDAV_URL = process.env.NZBDAV_URL || ''; // e.g. http://nzbdav-h80s8cwkssc8gos4ccw4w8k4:3000
const API_KEY = process.env.NZBDAV_API_KEY || '';
const WEBDAV_USER = process.env.NZBDAV_WEBDAV_USER || 'addon';
const WEBDAV_PASS = process.env.NZBDAV_WEBDAV_PASS || '';
const CATEGORY = process.env.NZBDAV_CATEGORY || 'stremio';
const POLL_TIMEOUT_MS = parseInt(process.env.NZBDAV_POLL_TIMEOUT_MS || '20000', 10);
const POLL_INTERVAL_MS = parseInt(process.env.NZBDAV_POLL_INTERVAL_MS || '500', 10);

function isConfigured() {
  return !!(NZBDAV_URL && API_KEY && WEBDAV_PASS);
}

/**
 * Deterministic job name from R2 URL.
 * Format: "<imdbHint>-<sha1-16>" — visible in NzbDav UI, idempotent across clicks.
 */
function makeJobName(r2Url, imdbHint = '') {
  const hash = crypto.createHash('sha1').update(r2Url).digest('hex').slice(0, 16);
  const prefix = imdbHint ? `${imdbHint.replace(/[^a-zA-Z0-9]/g, '').slice(0, 20)}-` : '';
  return `${prefix}${hash}`;
}

/**
 * Download NZB from R2 and return plain NZB XML buffer.
 * R2 storage holds both formats depending on source pipeline:
 *   - /nzb/    (NZBGeek)    → .nzb.gz (gzip-compressed)
 *   - /nzb-at/ (AnimeTosho) → may be either gzip OR plain XML (indexer-side variant)
 * Detect by gzip magic bytes (1f 8b) and act accordingly.
 * HTTP errors (404/403/etc.) are surfaced with body snippet for debugging.
 */
async function fetchNzb(r2Url) {
  let resp;
  try {
    resp = await axios.get(r2Url, {
      responseType: 'arraybuffer',
      timeout: 15000,
      validateStatus: () => true, // accept any status, handle below
      maxRedirects: 5,
    });
  } catch (err) {
    throw new Error(`fetchNzb: network error: ${err.message}`);
  }

  if (resp.status < 200 || resp.status >= 300) {
    const snippet = Buffer.from(resp.data || []).slice(0, 200).toString('utf8').replace(/\s+/g, ' ');
    throw new Error(`fetchNzb: HTTP ${resp.status} from ${r2Url.slice(0, 100)} — body: ${snippet}`);
  }

  const buf = Buffer.from(resp.data);
  if (!buf.length) throw new Error('fetchNzb: empty body');

  // Magic-byte detection: gzip starts with 0x1f 0x8b
  const isGzip = buf[0] === 0x1f && buf[1] === 0x8b;
  if (isGzip) {
    try {
      return zlib.gunzipSync(buf);
    } catch (err) {
      throw new Error(`fetchNzb: gunzip failed: ${err.message}`);
    }
  }

  // Not gzip — must be plain NZB XML
  const head = buf.slice(0, 200).toString('utf8');
  if (head.includes('<?xml') || head.includes('<nzb')) return buf;

  // Neither gzip nor XML — surface what we got
  const hexFirst4 = buf.slice(0, 4).toString('hex');
  const asciiFirst64 = buf.slice(0, 64).toString('utf8').replace(/[^\x20-\x7e]/g, '.');
  throw new Error(`fetchNzb: not gzip and not XML (first4=${hexFirst4}, ascii="${asciiFirst64}")`);
}

/**
 * POST NZB to NzbDav SAB API.
 * @param {Buffer} nzbBuf - plain NZB XML bytes
 * @param {string} jobName - desired stable name (passed as nzbname)
 * @returns {Promise<{nzo_id: string|null, duplicate: boolean}>}
 *   - duplicate:true means a job/folder with the same name already exists in NzbDav
 *     (downloading OR completed). Caller should skip upload and poll history.
 */
async function uploadNzb(nzbBuf, jobName) {
  const form = new FormData();
  form.append('name', nzbBuf, { filename: `${jobName}.nzb`, contentType: 'application/x-nzb' });

  const url = `${NZBDAV_URL}/api?mode=addfile&apikey=${encodeURIComponent(API_KEY)}&cat=${encodeURIComponent(CATEGORY)}&nzbname=${encodeURIComponent(jobName)}&output=json`;
  const resp = await axios.post(url, form, {
    headers: form.getHeaders(),
    timeout: 15000,
    maxContentLength: Infinity,
    maxBodyLength: Infinity,
    validateStatus: () => true, // we'll inspect the body
  });

  // Success
  if (resp.data?.status && Array.isArray(resp.data.nzo_ids) && resp.data.nzo_ids.length) {
    return { nzo_id: resp.data.nzo_ids[0], duplicate: false };
  }

  // Detect "already exists" — NzbDav returns status:false with an error message about duplicate.
  // This isn't a real failure; the job (or its folder) is already there from an earlier click.
  const errMsg = String(resp.data?.error || '').toLowerCase();
  if (errMsg.includes('duplicate') || errMsg.includes('already exist')) {
    return { nzo_id: null, duplicate: true };
  }

  throw new Error(`uploadNzb: bad response: ${JSON.stringify(resp.data).slice(0, 200)}`);
}

/**
 * Find all history entries matching a job name. Sorted newest-first (NzbDav order).
 */
async function findAllInHistory(jobName) {
  const url = `${NZBDAV_URL}/api?mode=history&apikey=${encodeURIComponent(API_KEY)}&output=json&limit=200`;
  const resp = await axios.get(url, { timeout: 8000 });
  const slots = resp.data?.history?.slots || resp.data?.slots || [];
  return slots.filter(s => s.name === jobName || s.nzb_name === jobName || s.nzo_id === jobName);
}

/**
 * Find a job in history by name. Returns the "best" matching slot, or null.
 * Priority: completed > in-progress > failed (with same name).
 * This is important because NzbDav often has multiple history entries for the same
 * nzbname — one completed job plus several failed-duplicate attempts. We want the
 * completed one to win.
 */
async function findInHistory(jobName) {
  const matches = await findAllInHistory(jobName);
  if (!matches.length) return null;
  const completed = matches.find(s => String(s.status || '').toLowerCase() === 'completed');
  if (completed) return completed;
  const inProgress = matches.find(s => {
    const st = String(s.status || '').toLowerCase();
    return st === 'downloading' || st === 'extracting' || st === 'queued' ||
           st === 'verifying' || st === 'repairing';
  });
  if (inProgress) return inProgress;
  return matches[0]; // fall back to most recent (probably failed)
}

/**
 * Delete a history entry by nzo_id. Used to clean up stale "failed" duplicates
 * before a fresh re-upload attempt.
 */
async function deleteHistoryEntry(nzoId) {
  if (!nzoId) return false;
  const url = `${NZBDAV_URL}/api?mode=history&name=delete&value=${encodeURIComponent(nzoId)}&apikey=${encodeURIComponent(API_KEY)}&output=json`;
  try {
    const resp = await axios.get(url, { timeout: 8000 });
    return !!resp.data?.status;
  } catch (e) {
    return false;
  }
}

/**
 * Poll history until job is Completed (or timeout). Returns the history slot.
 */
async function waitForCompleted(jobName, timeoutMs = POLL_TIMEOUT_MS) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const slot = await findInHistory(jobName);
    if (slot) {
      const status = String(slot.status || '').toLowerCase();
      if (status === 'completed') return slot;
      if (status === 'failed' || status === 'error') {
        throw new Error(`NzbDav job failed: ${slot.fail_message || jobName}`);
      }
    }
    await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
  }
  throw new Error(`NzbDav job timeout after ${timeoutMs}ms: ${jobName}`);
}

/**
 * PROPFIND on /content/<category>/<jobName>/ — returns list of video files (recursive).
 * Returns array of { name, path, size }.
 *
 * Uses Depth: infinity because NzbDav often creates Season subfolders for series
 * (e.g. /content/stremio/<jobName>/Season 1/episode.mkv). Without recursion we'd
 * only see the empty top-level folder.
 */
async function listVideoFiles(jobName) {
  const path = `/content/${encodeURIComponent(CATEGORY)}/${encodeURIComponent(jobName)}/`;
  const auth = 'Basic ' + Buffer.from(`${WEBDAV_USER}:${WEBDAV_PASS}`).toString('base64');
  const resp = await axios.request({
    method: 'PROPFIND',
    url: `${NZBDAV_URL}${path}`,
    headers: { Authorization: auth, Depth: 'infinity' },
    timeout: 15000,
    responseType: 'text',
    validateStatus: s => s === 207 || s === 200,
  });

  const xml = resp.data || '';
  // Parse <D:response> blocks. Each has <D:href>, <D:resourcetype>, <D:getcontentlength>.
  const responses = [];
  const re = /<D:response>([\s\S]*?)<\/D:response>/g;
  let m;
  while ((m = re.exec(xml)) !== null) {
    const block = m[1];
    const hrefM = block.match(/<D:href>([^<]+)<\/D:href>/);
    const isCollection = /<D:collection\s*\/?>/.test(block);
    const sizeM = block.match(/<D:getcontentlength>(\d+)<\/D:getcontentlength>/);
    if (!hrefM || isCollection) continue;

    // <D:href> contains internal URL like "http://localhost:8080/content/..." — ignore host, keep path.
    let hrefPath;
    try {
      hrefPath = new URL(hrefM[1]).pathname;
    } catch (e) {
      hrefPath = hrefM[1]; // already a path
    }
    // hrefPath is URL-encoded; decode for display, keep raw for proxy use
    const decoded = decodeURIComponent(hrefPath);
    const name = decoded.split('/').filter(Boolean).pop() || '';
    responses.push({
      name,
      path: hrefPath, // raw encoded — safe to append to NZBDAV_URL
      size: sizeM ? parseInt(sizeM[1], 10) : 0,
    });
  }

  // Video files only
  const videoExt = /\.(mkv|mp4|avi|m4v|mov|webm|ts)$/i;
  return responses.filter(f => videoExt.test(f.name));
}

/**
 * Pick the main video file from a list.
 * Heuristic: largest file (samples are tiny, main feature is biggest).
 */
function pickMainVideo(files) {
  if (!files.length) return null;
  return [...files].sort((a, b) => b.size - a.size)[0];
}

/**
 * Pick the matching video file for batch episodes.
 * Priority:
 *   1. Exact filename match (decoded file path basename == hints.n)
 *   2. Size within ±5% of hints.s (resilient to release-name renames)
 *   3. Largest file fallback (single-file NZB or unmatched batch)
 *
 * `hints` is `{ n: filename, s: size }` from indexer file_list, or null.
 */
function pickVideoForRequest(files, hints) {
  if (!files.length) return null;
  if (!hints || (!hints.n && !hints.s)) return pickMainVideo(files);

  // 1. Exact filename match — try basename match (file.name is already basename from listVideoFiles)
  if (hints.n) {
    const want = String(hints.n).trim();
    const exact = files.find(f => f.name === want);
    if (exact) return exact;
    // Try decoded name (some clients URL-encode while NzbDav keeps decoded, or vice versa)
    try {
      const decodedWant = decodeURIComponent(want);
      if (decodedWant !== want) {
        const exact2 = files.find(f => f.name === decodedWant);
        if (exact2) return exact2;
      }
    } catch (e) {}
  }

  // 2. Size match — pick file within ±5% of expected size
  if (hints.s && hints.s > 0) {
    const target = hints.s;
    const tol = target * 0.05;
    const sizeMatch = files
      .filter(f => Math.abs(f.size - target) <= tol)
      .sort((a, b) => Math.abs(a.size - target) - Math.abs(b.size - target))[0];
    if (sizeMatch) return sizeMatch;
  }

  // 3. Last resort: largest
  return pickMainVideo(files);
}

/**
 * Range-proxy a WebDAV file to the client response.
 * Forwards Range, returns 206 / 200 with proper headers, streams bytes.
 */
async function proxyStream(req, res, filePath) {
  const auth = 'Basic ' + Buffer.from(`${WEBDAV_USER}:${WEBDAV_PASS}`).toString('base64');
  const headers = { Authorization: auth };
  if (req.headers.range) headers.Range = req.headers.range;
  if (req.headers['user-agent']) headers['User-Agent'] = req.headers['user-agent'];

  let upstream;
  try {
    upstream = await axios.request({
      method: 'GET',
      url: `${NZBDAV_URL}${filePath}`,
      headers,
      responseType: 'stream',
      // Accept any 2xx/3xx/4xx so we can forward errors too
      validateStatus: () => true,
      timeout: 30000,
    });
  } catch (err) {
    if (!res.headersSent) res.status(502).send(`NzbDav upstream error: ${err.message}`);
    return;
  }

  // Forward status + selected headers
  res.status(upstream.status);
  const passthrough = ['content-type', 'content-length', 'content-range', 'accept-ranges', 'last-modified', 'etag'];
  for (const h of passthrough) {
    const v = upstream.headers[h];
    if (v) res.setHeader(h, v);
  }
  // Ensure these are always set even if upstream forgot — some clients refuse to play without them.
  if (!upstream.headers['accept-ranges']) res.setHeader('Accept-Ranges', 'bytes');
  if (!upstream.headers['content-type']) {
    const lower = filePath.toLowerCase();
    let ct = 'application/octet-stream';
    if (lower.endsWith('.mkv')) ct = 'video/x-matroska';
    else if (lower.endsWith('.mp4') || lower.endsWith('.m4v')) ct = 'video/mp4';
    else if (lower.endsWith('.webm')) ct = 'video/webm';
    else if (lower.endsWith('.avi')) ct = 'video/x-msvideo';
    else if (lower.endsWith('.mov')) ct = 'video/quicktime';
    else if (lower.endsWith('.ts')) ct = 'video/mp2t';
    res.setHeader('Content-Type', ct);
  }
  // Ensure no caching by intermediate proxies
  res.setHeader('Cache-Control', 'no-store');

  upstream.data.on('error', (err) => {
    console.log(`  ⚠️ NzbDav stream error: ${err.message}`);
    if (!res.writableEnded) res.end();
  });
  req.on('close', () => {
    if (upstream.data && typeof upstream.data.destroy === 'function') upstream.data.destroy();
  });

  upstream.data.pipe(res);
}

/**
 * Build the proxy URL path component for a job's video file.
 * If `hints` provided ({n: filename, s: size}), picks that specific file (for batches).
 * Otherwise falls back to the largest file (single-episode default).
 */
async function resolveStreamPath(jobName, hints = null) {
  const files = await listVideoFiles(jobName);
  const main = pickVideoForRequest(files, hints);
  return main ? main.path : null;
}

/**
 * Full resolve flow for a stream click: ensure NZB is uploaded + completed + ready to stream.
 * Returns the WebDAV path of the matching video file, ready to be passed to proxyStream.
 *
 * Idempotent on (r2Url, hints): two calls with the same args resolve to the same WebDAV file.
 *
 * @param {string} r2Url
 * @param {string} imdbHint - optional prefix for the jobName (visibility in NzbDav UI)
 * @param {object|null} hints - { n: filename, s: size } for batch file selection, or null
 * @returns {Promise<{jobName: string, filePath: string}>}
 */
// In-flight mutex: ensure that concurrent ensureReady calls for the same jobName
// share a single underlying fetch/upload/poll, preventing duplicate-upload errors
// when Stremio clicks multiple episodes from the same batch in quick succession.
const inFlightJobs = new Map(); // jobName → Promise<{jobName, filePath: null}>

async function ensureReady(r2Url, imdbHint = '', hints = null) {
  if (!isConfigured()) throw new Error('NzbDav not configured');
  const jobName = makeJobName(r2Url, imdbHint);

  // De-duplicate concurrent calls for this jobName.
  // The in-flight promise resolves once the job is `completed` in NzbDav history.
  // We then resolve the file path separately per caller, since `hints` may differ.
  let jobReadyP = inFlightJobs.get(jobName);
  if (!jobReadyP) {
    jobReadyP = (async () => {
      const t0 = Date.now();

      // Check history for any existing job with this name.
      // (findInHistory prefers completed over failed when multiple entries exist.)
      const existing = await findInHistory(jobName);
      const existingStatus = existing ? String(existing.status || '').toLowerCase() : '';

      if (existingStatus === 'completed') {
        // Opportunistic cleanup: NzbDav may also hold several failed-duplicate entries
        // from earlier attempts. Sweep them so the history stays tidy. Best-effort.
        try {
          const all = await findAllInHistory(jobName);
          const stale = all.filter(s => {
            const st = String(s.status || '').toLowerCase();
            const fm = String(s.fail_message || '').toLowerCase();
            return (st === 'failed' || st === 'error') &&
                   (fm.includes('duplicate') || fm.includes('already exist'));
          });
          for (const s of stale) await deleteHistoryEntry(s.nzo_id);
          if (stale.length) console.log(`  📡 NzbDav: ${jobName} cleaned ${stale.length} stale failed-duplicate entries`);
        } catch (e) {}
        return; // already done — caller resolves filePath below
      }

      if (existing && (existingStatus === 'downloading' || existingStatus === 'extracting' ||
                       existingStatus === 'queued' || existingStatus === 'verifying' ||
                       existingStatus === 'repairing')) {
        // Job is in progress — just wait for completion, don't re-upload
        console.log(`  📡 NzbDav: ${jobName} already in progress (${existingStatus}), waiting…`);
        await waitForCompleted(jobName);
        console.log(`  📡 NzbDav: ${jobName} ready (existing job) in ${Date.now() - t0}ms`);
        return;
      }

      if (existing && (existingStatus === 'failed' || existingStatus === 'error')) {
        const failMsg = String(existing.fail_message || '').toLowerCase();
        const isDupFail = failMsg.includes('duplicate') || failMsg.includes('already exist');

        if (isDupFail) {
          // The folder for this NZB already exists (likely a completed job whose
          // history entry got rotated out, or a leftover from a partial earlier run).
          // First, probe WebDAV — if there's a video file there, we can use it directly.
          console.log(`  📡 NzbDav: ${jobName} has stale failed-duplicate entry, probing WebDAV…`);
          try {
            const files = await listVideoFiles(jobName);
            if (files.length) {
              // Folder exists with video files → clean the failed history entry and treat as ready.
              await deleteHistoryEntry(existing.nzo_id);
              console.log(`  📡 NzbDav: ${jobName} folder already populated (${files.length} files), reusing`);
              return;
            }
          } catch (e) {
            // PROPFIND failed, fall through to delete + re-upload
          }
          // No video in folder → clean failed entry and re-upload
          console.log(`  📡 NzbDav: ${jobName} duplicate but no video files, deleting failed entry and re-uploading`);
          await deleteHistoryEntry(existing.nzo_id);
          // Fall through to upload path below
        } else {
          throw new Error(`NzbDav job in failed state: ${existing.fail_message || jobName}`);
        }
      }

      // No (usable) history entry → fetch + upload + poll
      const nzb = await fetchNzb(r2Url);
      const upRes = await uploadNzb(nzb, jobName);
      if (upRes.duplicate) {
        // Duplicate means the folder already exists. Two cases:
        //   (a) Concurrent in-progress job (history just not visible yet) — wait
        //   (b) Completed-and-rotated job (folder exists, no history entry) — skip wait
        // Probe WebDAV: if there's a video file, case (b) — use it.
        try {
          const files = await listVideoFiles(jobName);
          if (files.length) {
            console.log(`  📡 NzbDav: ${jobName} folder already populated after duplicate, reusing`);
            return;
          }
        } catch (e) {}
        console.log(`  📡 NzbDav: ${jobName} duplicate upload, waiting for concurrent job…`);
      }
      await waitForCompleted(jobName);
      console.log(`  📡 NzbDav: ${jobName} ready (fresh upload) in ${Date.now() - t0}ms`);
    })();
    inFlightJobs.set(jobName, jobReadyP);
    // Always clean up the in-flight entry once settled
    jobReadyP.finally(() => {
      if (inFlightJobs.get(jobName) === jobReadyP) inFlightJobs.delete(jobName);
    });
  }

  // Wait for the shared job-ready promise. If it threw, propagate.
  await jobReadyP;

  // Resolve filePath per caller (different hints → potentially different file in same batch).
  const filePath = await resolveStreamPath(jobName, hints);
  if (!filePath) throw new Error(`No video file in NzbDav job ${jobName}`);
  return { jobName, filePath };
}

module.exports = {
  isConfigured,
  makeJobName,
  ensureReady,
  proxyStream,
  // Exposed for tests/debug
  fetchNzb,
  uploadNzb,
  findInHistory,
  findAllInHistory,
  deleteHistoryEntry,
  waitForCompleted,
  listVideoFiles,
  pickMainVideo,
  pickVideoForRequest,
};
