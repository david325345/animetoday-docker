...
async function ensureReady(r2Url, imdbHint = '', hints = null) {
  ...
  if (!jobReadyP) {
    jobReadyP = (async () => {
      ...
      // No history entry → fetch + upload + poll
      const nzb = await fetchNzb(r2Url);
      const upRes = await uploadNzb(nzb, jobName);
      if (upRes.duplicate) {
        // Race: another request created the job between our findInHistory and uploadNzb.
        // Treat as success — just wait for completion.
        console.log(`  📡 NzbDav: ${jobName} already uploaded by concurrent request, waiting…`);
      }
      await waitForCompleted(jobName);
