...
async function findMediaId(animeName, apiKey) {
  if (!animeName || !apiKey) { console.log(`  🐱 NekoBT findMediaId: skip (name=${!!animeName}, key=${!!apiKey})`); return null; }

  const cacheKey = animeName.toLowerCase().trim();
  if (mediaIdCache.has(cacheKey)) {
    const cached = mediaIdCache.get(cacheKey);
    console.log(`  🐱 NekoBT findMediaId: cache hit "${animeName}" → ${cached || 'null'}`);
    return cached;
  }

  try {
    console.log(`  🐱 NekoBT findMediaId: searching "${animeName}"...`);
    const client = nekobtAxios(apiKey);
    const resp = await client.get('/media/search', { params: { query: animeName, limit: 5 } });

    // Response: { error: false, data: { more: bool, results: [...] } }
    const rawData = resp.data?.data;
    console.log(`  🐱 NekoBT findMediaId: response type=${typeof rawData}, isArray=${Array.isArray(rawData)}, keys=${rawData ? Object.keys(rawData).join(',') : 'null'}`);

    let results = rawData;
    if (results && !Array.isArray(results) && results.results) results = results.results;
    if (!Array.isArray(results) || !results.length) {
      console.log(`  🐱 NekoBT: no media for "${animeName}" (results=${JSON.stringify(results)?.slice(0, 100)})`);
    ...
    const mediaId = media.id; // e.g. "s462"
    console.log(`  🐱 NekoBT: "${animeName}" → ${mediaId} "${media.title}"`);

    mediaIdCache.set(cacheKey, mediaId);
    return mediaId;
  } catch (err) {
    console.log(`  🐱 NekoBT findMediaId ERROR: ${err.message} (status=${err.response?.status}, code=${err.code})`);
    if (err.response?.data) console.log(`  🐱 NekoBT findMediaId response: ${JSON.stringify(err.response.data).slice(0, 200)}`);
    return null;
  }
}
...
async function searchByMediaId(mediaId, apiKey, options = {}) {
  ...
  try {
    ...
    // Response: { error: false, data: { results: [...], more: bool, ... } }
    ...
    const torrents = results
      .map(t => mapTorrent(t))
      .filter(t => t.magnet); // Only keep torrents with magnets

    console.log(`  🐱 NekoBT: ${torrents.length} torrents with magnets for ${mediaId} (${results.length} total)`);
...
async function findEpisodeIds(mediaId, apiKey, seasonNum, episodeNum) {
  ...
  if (matches.length) return matches.map(ep => String(ep.id));
  ...
  if (absMatches.length) return absMatches.map(ep => String(ep.id));

  // Fallback: match by absolute position
  const absByIndex = detail.episodes.filter(ep => ep.absolute === episodeNum);
  if (absByIndex.length) return absByIndex.map(ep => String(ep.id));
...
async function searchNekobt(names, apiKey, season, episode, isMovie) {
  if (!apiKey || !names?.length) { console.log(`  🐱 NekoBT searchNekobt: skip (key=${!!apiKey}, names=${names?.length})`); return []; }

  try {
    // 1. Find media_id
    console.log(`  🐱 NekoBT pipeline: starting for [${names.join(', ')}]`);
    let mediaId = null;
    for (const name of names) {
      mediaId = await findMediaId(name, apiKey);
      if (mediaId) break;
    }
    if (!mediaId) { console.log(`  🐱 NekoBT pipeline: no media_id found, giving up`); return []; }

    // 2. Find episode IDs (if not a movie)
    let episodeIds = [];
    if (!isMovie && episode) {
      episodeIds = await findEpisodeIds(mediaId, apiKey, season || 1, episode);
      if (!episodeIds.length && (season || 1) !== 0) {
    ...
    console.log(`  🐱 NekoBT searchNekobt FATAL ERROR: ${err.message}`);
    console.log(`  🐱 NekoBT stack: ${err.stack?.split('\n').slice(0, 3).join(' | ')}`);
    return [];
  }
}
