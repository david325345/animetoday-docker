...
function loadAccounts() {
  ...
  if (!accounts[SUPER_ADMIN.username]) {
    ...
    accounts[SUPER_ADMIN.username] = {
      ...
      token,
      role: 'superadmin',
      permissions: { torrents: true, nzb: true, subtitles: true, catalog: true, ondemand: true },
...
function createAccount(username, password, permissions = {}) {
  ...
  const token = crypto.randomBytes(8).toString('hex');
  const defaultPerms = { torrents: true, nzb: false, subtitles: false, catalog: false, ondemand: true };
  accounts[username] = {
    password,
    token,
    role: 'user',
