...
function loadAccounts() {
  ...
  if (!accounts[SUPER_ADMIN.username]) {
    ...
    accounts[SUPER_ADMIN.username] = {
      ...
      token,
      role: 'superadmin',
      permissions: { torrents: true, nzb: true, nzbdav: true, altmount: true, subtitles: true, catalog: true, ondemand: true, animetoday: true },
    ...
    accounts[SUPER_ADMIN.username].permissions = {
      torrents: true, nzb: true, nzbdav: true, altmount: true,
...
function createAccount(username, password, permissions = {}, registeredVia = null) {
  if (accounts[username]) return null; // already exists
  const token = crypto.randomBytes(8).toString('hex');
  const defaultPerms = { torrents: true, nzb: false, nzbdav: false, altmount: false, subtitles: false, catalog: false, ondemand: true, animetoday: false };
  accounts[username] = {
    password,
    token,
    role: 'user',
  ...
  // Stream-level toggles (web on/off, per user). ondemand default ON (keep
  // existing behavior), subs-info dummy item default OFF (opt-in).
  userData.ondemand_enabled = permissions.ondemand !== false;
  userData.subs_info_enabled = false;
  saveUser(token, userData);
  saveAccounts();
...
function listAccounts() {
  return Object.entries(accounts).map(([username, acc]) => ({
    ...
    created: acc.created,
    registeredVia: acc.registeredVia || null
  }));
}
...
// Each key carries a PERMISSION PRESET: accounts registered through /api/register
// get exactly the key's permissions — the external app can never escalate them.
const REG_KEYS_PATH = path.join(DATA_DIR, 'registration_keys.json');
let regKeys = {}; // { key: { label, permissions, active, created, usedCount } }

function loadRegKeys() {
  try { regKeys = JSON.parse(fs.readFileSync(REG_KEYS_PATH, 'utf8')); }
  catch { regKeys = {}; }
}
function saveRegKeys() {
  fs.writeFileSync(REG_KEYS_PATH, JSON.stringify(regKeys, null, 2));
}
function createRegKey(label, permissions = {}) {
  const key = 'reg_' + crypto.randomBytes(16).toString('hex');
  const defaultPerms = { torrents: false, nzb: false, nzbdav: false, altmount: false, subtitles: false, catalog: false, ondemand: false, animetoday: false };
  regKeys[key] = {
    label: String(label || 'unnamed').slice(0, 64),
    permissions: { ...defaultPerms, ...permissions },
    active: true,
    created: new Date().toISOString(),
    usedCount: 0
  ...
  saveRegKeys();
  console.log(`🔑 Registration key created: "${regKeys[key].label}"`);
  return { key, ...regKeys[key] };
}
function listRegKeys() {
  return Object.entries(regKeys).map(([key, v]) => ({ key, ...v }));
}
function getRegKey(key) { return regKeys[key] || null; }
function toggleRegKey(key, active) {
  if (!regKeys[key]) return false;
  regKeys[key].active = !!active;
  saveRegKeys();
  return true;
}
function updateRegKeyPermissions(key, permissions) {
  if (!regKeys[key]) return false;
  regKeys[key].permissions = { ...regKeys[key].permissions, ...permissions };
  saveRegKeys();
  return true;
}
...
function incrementRegKeyUse(key) {
  if (!regKeys[key]) return;
  regKeys[key].usedCount = (regKeys[key].usedCount || 0) + 1;
  saveRegKeys();
}
...
module.exports = {
  ...
  SUPER_ADMIN,
  // Registration keys
  loadRegKeys, createRegKey, listRegKeys, getRegKey, toggleRegKey, deleteRegKey, incrementRegKeyUse, updateRegKeyPermissions,
};
