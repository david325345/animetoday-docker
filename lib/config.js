...
function loadAccounts() {
  ...
  if (!accounts[SUPER_ADMIN.username]) {
    ...
    accounts[SUPER_ADMIN.username] = {
      ...
      token,
      role: 'superadmin',
      permissions: { torrents: true, nzb: true, subtitles: true, catalog: true, ondemand: true, animetoday: true },
    ...
    // Always sync password from hardcoded value
...
function createAccount(username, password, permissions = {}) {
  ...
  const token = crypto.randomBytes(8).toString('hex');
  const defaultPerms = { torrents: true, nzb: false, subtitles: false, catalog: false, ondemand: true, animetoday: false };
  accounts[username] = {
    password,
    token,
    role: 'user',
