...
// ===== Get TorBox NZB stream (full flow) =====
// hint: optional { name, size } from indexer's matchedFile — same purpose as in
//       getTBStream: pick the right file in a batch NZB by content (basename /
//       size) instead of by index. NZB file ordering on TB is no more reliable
//       than torrent file ordering.
async function getTBNZBStream(nzbUrl, apiKey, episode = 0, torrentName = '', hint = null) {
  if (!apiKey || !nzbUrl) return null;

  // Include hint name in cache key so different files of the same NZB batch
  // don't collide (same nzbUrl + episode but different target file).
  const hintKey = hint?.name ? `:n=${hint.name}` : '';
  const cacheKey = `tb-nzb:${nzbUrl}:${episode}${hintKey}`;
  const cached = tbStreamCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < TB_CACHE_TTL) return cached.url;
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (info.download_finished || info.download_state === 'completed' || info.files?.length) {
        // Reuse pickTBFile so NZB and torrent file selection share identical
        // priority logic (basename > size > size±0.1% > regex/index fallback).
        // For NZB we never use the fileIdx path (no isFileIdx), so the function
        // falls through to the regex/index fallback when there's no hint.
        const targetFile = pickTBFile(info.files || [], episode, false, hint);
        if (!targetFile) continue;

        const url = await getNZBStreamLink(apiKey, nzbId, targetFile.id);
        if (url) {
          console.log(`  TB: ✅ NZB ready (file: ${targetFile.short_name || targetFile.name})`);
          tbStreamCache.set(cacheKey, { url, timestamp: Date.now() });
          return url;
        }
      }
