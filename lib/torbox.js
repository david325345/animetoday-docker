...
async function addNZB(apiKey, nzbUrl) {
  try {
    ...
    form.append('file', nzbData, { filename: 'upload.nzb', contentType: 'application/x-nzb' });
...
async function getTBNZBStream(nzbUrl, apiKey, episode = 0) {
  ...
  try {
    const nzbId = await addNZB(apiKey, nzbUrl);
