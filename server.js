...
function decodeSubsText(buf) {
  ...
  try { return utf8Strict.decode(buf).replace(/^\uFEFF/, ''); } catch {}
  let out = '';
  for (const b of buf) out += b < 0x80 ? String.fromCharCode(b) : String.fromCharCode(CP1250_MAP.get(b) ?? b);
  return out;
}
...
app.get('/:token/subs/file/:b64.ass', async (req, res) => {
  ...
  try {
    ...
    if (cached && Date.now() - cached.ts < SUBS_CACHE_TTL) {
      ...
      plain = Buffer.from(decodeSubsText(gunzipSmart(Buffer.from(r.data))), 'utf8');
    ...
    res.set('Cache-Control', 'public, max-age=3600'); // 24h → 1h: po opravách obsahu (charset) nesmí klienti den servírovat starou verzi
    res.send(plain);
  } catch (err) {
...
app.get('/:token/subs/file/:b64.vtt', async (req, res) => {
  ...
  try {
    ...
    if (cached && Date.now() - cached.ts < SUBS_CACHE_TTL) {
      ...
      const r = await axios.get(gzUrl, { responseType: 'arraybuffer', timeout: 15000 });
      const plain = decodeSubsText(gunzipSmart(Buffer.from(r.data)));
  ...
  if (!hasExplicitEpisode && !isMovie) {
    ...
    if (imdbBase) {
      ...
      for (const s of todayAnimeCache) {
        ...
        if (rec?.imdb === imdbBase) { episode = s.episode; season = s.season || 1; matchedByImdb = true; console.log(`  📅 Today (imdb): ${imdbBase} → S${season}E${episode}`); break; }
      }
      ...
      if (!matchedByImdb) {
        try {
          ...
          if (resolved?.anidb) {
            for (const s of todayAnimeCache) {
              const rec = offlineDB.byAniList.get(s.anilistId);
              if (rec?.anidb === resolved.anidb) { episode = s.episode; season = s.season || 1; matchedByImdb = true; console.log(`  📅 Today (anidb): ${imdbBase} → AniDB ${resolved.anidb} → S${season}E${episode}`); break; }
            }
          }
        } catch {}
      }
    ...
    if (!matchedByImdb) {
      for (const s of todayAnimeCache) {
        const rec = offlineDB.byAniList.get(s.anilistId);
        if (kitsuId && rec?.kitsu === kitsuId) { episode = s.episode; season = s.season || 1; console.log(`  📅 Today: kitsu:${kitsuId} → S${season}E${episode}`); break; }
        if (anilistIdNum && s.anilistId === anilistIdNum) { episode = s.episode; season = s.season || 1; console.log(`  📅 Today: anilist:${anilistIdNum} → S${season}E${episode}`); break; }
      }
    }
  }
