...
// ===== Nyaa.si search =====
async function searchNyaaSingle(animeName, episode, season = 1) {
  const cacheKey = `nyaa:${animeName}:${episode}:s${season}`;
  const cached = nyaaCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < NYAA_CACHE_TTL) return cached.data;

  const variants = buildSearchVariants(animeName, episode, season);
  const nameOnly = buildSearchVariants(animeName, null);
  const allVariants = [...new Set([...variants, ...nameOnly])];
  ...
  let filtered = filterJunk(allTorrents, animeName);
...
// Search across multiple name variants
async function searchNyaa(names, episode, season = 1) {
  ...
  for (const name of names) {
    if (!name) continue;
    console.log(`  🔍 Nyaa: "${name}" ep${episode || '?'} s${season}`);
    const torrents = await searchNyaaSingle(name, episode, season);
    console.log(`  [Nyaa.si] "${name}" → ${torrents.length} results`);
  ...
  // Sukebei fallback
  if (combined.length === 0 && names.length > 0) {
    console.log('  ⚠️ Trying Sukebei...');
    const sukebei = await searchSukebei(names, episode);
    combined.push(...sukebei);
  }

  const sorted = combined.sort((a, b) => (parseInt(b.seeders) || 0) - (parseInt(a.seeders) || 0));
